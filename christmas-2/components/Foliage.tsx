...
export const Foliage: React.FC<FoliageProps> = ({ mode, count }) => {
  ...
  const { chaosPositions, targetPositions, randoms } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 2. Target Positions: Spiral Cone with organic variation
      // Normalized height 0 to 1
      const yNorm = i / count; 
      const y = yNorm * height;
      
      // Add natural shape curve - wider at bottom, slightly curved inward at top
      // Quadratic bezier-like curve for radius profile
      const shapeProfile = Math.pow(1 - yNorm, 1.2); 
      const currentRadius = maxRadius * shapeProfile;
      
      const angle = 2 * Math.PI * goldenRatio * i;
      
      // Add randomness to position for "fluffy" look
      // Radial noise (in/out)
      const radialNoise = (Math.random() - 0.5) * 0.8 * (1 - yNorm * 0.5); 
      // Vertical noise (up/down)
      const verticalNoise = (Math.random() - 0.5) * 0.5;
      
      // Branch clustering effect
      // Group particles into "branches" using sine waves on the angle
      const branchNoise = Math.sin(angle * 5.0 + yNorm * 10.0) * 0.3 * (1 - yNorm);
      
      const finalRadius = currentRadius + radialNoise + branchNoise;
      
      target[i * 3] = Math.cos(angle) * finalRadius;
      target[i * 3 + 1] = y + verticalNoise;
      target[i * 3 + 2] = Math.sin(angle) * finalRadius;
